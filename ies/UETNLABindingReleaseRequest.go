...
func (msg *UETNLABindingReleaseRequest) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("UETNLABindingReleaseRequest"), err)
		}
	}()
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := UETNLABindingReleaseRequestDecoder{
		msg:  msg,
...
type UETNLABindingReleaseRequestDecoder struct {
	msg      *UETNLABindingReleaseRequest
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *UETNLABindingReleaseRequestDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
