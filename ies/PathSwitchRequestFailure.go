...
func (msg *PathSwitchRequestFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	...
	if _, ok := decoder.list[ProtocolIEID_PDUSessionResourceReleasedListPSFail]; !ok {
		err = fmt.Errorf("Mandatory field PDUSessionResourceReleasedListPSFail is missing")
		decoder.diagList = append(decoder.diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: Criticality_PresentIgnore},
			IEID:          ProtocolIEID{Value: ProtocolIEID_PDUSessionResourceReleasedListPSFail},
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
		return
	}
...
type PathSwitchRequestFailureDecoder struct {
	msg      *PathSwitchRequestFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *PathSwitchRequestFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
