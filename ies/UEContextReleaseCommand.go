...
func (msg *UEContextReleaseCommand) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	...
	if _, ok := decoder.list[ProtocolIEID_UENGAPIDs]; !ok {
		err = fmt.Errorf("Mandatory field UENGAPIDs is missing")
		decoder.diagList = append(decoder.diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: Criticality_PresentReject},
			IEID:          ProtocolIEID{Value: ProtocolIEID_UENGAPIDs},
...
func (decoder *UEContextReleaseCommandDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
