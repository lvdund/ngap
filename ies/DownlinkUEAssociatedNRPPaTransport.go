...
func (msg *DownlinkUEAssociatedNRPPaTransport) Encode(w io.Writer) (err error) {
	var ies []NgapMessageIE
	if ies, err = msg.toIes(); err != nil {
		return
	}
	return encodeMessage(w, NgapPduInitiatingMessage, ProcedureCode_DownlinkUEAssociatedNRPPaTransport, Criticality_PresentIgnore, ies)
}
func (msg *DownlinkUEAssociatedNRPPaTransport) toIes() (ies []NgapMessageIE, err error) {
	ies = []NgapMessageIE{}
...
func (msg *DownlinkUEAssociatedNRPPaTransport) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("DownlinkUEAssociatedNRPPaTransport"), err)
		}
	}()
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := DownlinkUEAssociatedNRPPaTransportDecoder{
		msg:  msg,
...
type DownlinkUEAssociatedNRPPaTransportDecoder struct {
	msg      *DownlinkUEAssociatedNRPPaTransport
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *DownlinkUEAssociatedNRPPaTransportDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
