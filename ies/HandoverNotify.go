...
func (msg *HandoverNotify) Encode(w io.Writer) (err error) {
	var ies []NgapMessageIE
	if ies, err = msg.toIes(); err != nil {
		return
	}
	return encodeMessage(w, NgapPduInitiatingMessage, ProcedureCode_HandoverNotification, Criticality_PresentIgnore, ies)
}
func (msg *HandoverNotify) toIes() (ies []NgapMessageIE, err error) {
	ies = []NgapMessageIE{}
...
func (msg *HandoverNotify) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("HandoverNotify"), err)
		}
	}()
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := HandoverNotifyDecoder{
		msg:  msg,
...
type HandoverNotifyDecoder struct {
	msg      *HandoverNotify
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *HandoverNotifyDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
