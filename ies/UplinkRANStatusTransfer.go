...
func (msg *UplinkRANStatusTransfer) toIes() (ies []NgapMessageIE, err error) {
	ies = []NgapMessageIE{}
...
func (msg *UplinkRANStatusTransfer) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := UplinkRANStatusTransferDecoder{
		msg:  msg,
...
type UplinkRANStatusTransferDecoder struct {
	msg      *UplinkRANStatusTransfer
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *UplinkRANStatusTransferDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
