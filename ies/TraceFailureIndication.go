...
func (msg *TraceFailureIndication) Encode(w io.Writer) (err error) {
	var ies []NgapMessageIE
	if ies, err = msg.toIes(); err != nil {
		return
	}
	return encodeMessage(w, NgapPduInitiatingMessage, ProcedureCode_TraceFailureIndication, Criticality_PresentIgnore, ies)
}
func (msg *TraceFailureIndication) toIes() (ies []NgapMessageIE, err error) {
	ies = []NgapMessageIE{}
...
func (msg *TraceFailureIndication) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("TraceFailureIndication"), err)
		}
	}()
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := TraceFailureIndicationDecoder{
		msg:  msg,
...
type TraceFailureIndicationDecoder struct {
	msg      *TraceFailureIndication
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *TraceFailureIndicationDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
