...
func (msg *UEContextModificationFailure) Encode(w io.Writer) (err error) {
	...
	return encodeMessage(w, NgapPduUnsuccessfulOutcome, ProcedureCode_UEContextModification, Criticality_PresentReject, ies)
}
func (msg *UEContextModificationFailure) toIes() (ies []NgapMessageIE, err error) {
	ies = []NgapMessageIE{}
...
func (msg *UEContextModificationFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
			err = msgErrors(fmt.Errorf("UEContextModificationFailure"), err)
		}
	}()
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := UEContextModificationFailureDecoder{
		msg:  msg,
...
type UEContextModificationFailureDecoder struct {
	msg      *UEContextModificationFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *UEContextModificationFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
