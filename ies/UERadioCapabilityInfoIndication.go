...
func (msg *UERadioCapabilityInfoIndication) toIes() (ies []NgapMessageIE, err error) {
	ies = []NgapMessageIE{}
...
func (msg *UERadioCapabilityInfoIndication) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	...
	if _, ok := decoder.list[ProtocolIEID_UERadioCapability]; !ok {
		err = fmt.Errorf("Mandatory field UERadioCapability is missing")
		decoder.diagList = append(decoder.diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: Criticality_PresentIgnore},
			IEID:          ProtocolIEID{Value: ProtocolIEID_UERadioCapability},
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
		return
	}
...
type UERadioCapabilityInfoIndicationDecoder struct {
	msg      *UERadioCapabilityInfoIndication
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *UERadioCapabilityInfoIndicationDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
