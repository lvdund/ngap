...
func (msg *RANConfigurationUpdateAcknowledge) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	defer func() {
		if err != nil {
	...
	r.ReadBool()
	decoder := RANConfigurationUpdateAcknowledgeDecoder{
		msg:  msg,
...
type RANConfigurationUpdateAcknowledgeDecoder struct {
	msg      *RANConfigurationUpdateAcknowledge
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *RANConfigurationUpdateAcknowledgeDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
