...
func (msg *RRCInactiveTransitionReport) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	...
	if _, ok := decoder.list[ProtocolIEID_RRCState]; !ok {
		err = fmt.Errorf("Mandatory field RRCState is missing")
		decoder.diagList = append(decoder.diagList, CriticalityDiagnosticsIEItem{
			IECriticality: Criticality{Value: Criticality_PresentIgnore},
			IEID:          ProtocolIEID{Value: ProtocolIEID_RRCState},
			TypeOfError:   TypeOfError{Value: TypeOfErrorMissing},
		})
		return
	}
...
type RRCInactiveTransitionReportDecoder struct {
	msg      *RRCInactiveTransitionReport
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *RRCInactiveTransitionReportDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
	...
	case ProtocolIEID_RRCState:
		var tmp RRCState
		if err = tmp.Decode(ieR); err != nil {
			err = utils.WrapError("Read RRCState", err)
			return
		}
		msg.RRCState = tmp
