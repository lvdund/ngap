...
func (msg *RANConfigurationUpdateFailure) Encode(w io.Writer) (err error) {
	return encodeMessage(w, NgapPduUnsuccessfulOutcome, ProcedureCode_RANConfigurationUpdate, Criticality_PresentReject, msg.toIes())
}
func (msg *RANConfigurationUpdateFailure) toIes() (ies []NgapMessageIE) {
	ies = []NgapMessageIE{}
...
func (msg *RANConfigurationUpdateFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := RANConfigurationUpdateFailureDecoder{
		msg:  msg,
...
type RANConfigurationUpdateFailureDecoder struct {
	msg      *RANConfigurationUpdateFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *RANConfigurationUpdateFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
