...
func (msg *HandoverFailure) Encode(w io.Writer) (err error) {
	return encodeMessage(w, NgapPduUnsuccessfulOutcome, ProcedureCode_HandoverResourceAllocation, Criticality_PresentReject, msg.toIes())
}
func (msg *HandoverFailure) toIes() (ies []NgapMessageIE) {
	ies = []NgapMessageIE{}
...
func (msg *HandoverFailure) Decode(wire []byte) (err error, diagList []CriticalityDiagnosticsIEItem) {
	r := aper.NewReader(bytes.NewReader(wire))
	r.ReadBool()
	decoder := HandoverFailureDecoder{
		msg:  msg,
...
type HandoverFailureDecoder struct {
	msg      *HandoverFailure
	diagList []CriticalityDiagnosticsIEItem
	list     map[aper.Integer]*NgapMessageIE
}

func (decoder *HandoverFailureDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	var id int64
	var c uint64
	var buf []byte
